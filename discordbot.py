...
def main(manual_args=None, source_img=None):
    ...
    # gather up all prompts, deduplicate. list(set([...])) seems to reverse the initial order?
    all_prompts_for_grid = dedup_list(flatten_sublists(data))
    ...
    with torch.no_grad():
        ...
        for n in trange(opt.n_iter, desc="Sampling"):
            ...
            GRID_PATH = os.path.join(OUTPUTS_DIR, f'{base_filename}.png')
            grid_img.save(GRID_PATH, pnginfo=all_prompts_metadata)
...
# remove duplicate items from a list while keeping the order intact.
def dedup_list(l:list) -> list:
    already_added = set()
    add_new = already_added.add
    # if an item is already in the collected set, ignore it. If it is not, add it to both the set and the accumulated list
    return [x for x in l if not (x in already_added or add_new(x))]    
...
async def on_message(message: discord.Message):
    ...
    if message.content.startswith(COMMAND_PREFIX):
        try:
            ...
            # split command into args, ignore empty elements caused by multiple spaces.
            separated_command = command.split('"')
            if not len(separated_command) == 3:
            ...
            print(sys.exc_info())
            await message.channel.send("ERROR: something went terribly wrong!")
